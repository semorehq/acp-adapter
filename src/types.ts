// ACP transport schema — framework-agnostic subset.
// Wire-format mirrors the internal monorepo definition at apps/api/src/routes/acp.ts;
// only the public-facing shape is re-declared here for OSS consumers.
...
export interface ACPToken {
  ...
  readonly amount: {
    ...
    readonly value: string; // decimal string — avoids float drift
  };
  /** Optional cart binding. When present the merchant MUST validate cart match. */
  readonly cart?: {
    readonly id: string;
    readonly hash: string; // sha256 of canonical cart JSON
  };
...
export type VerifyResult =
  | { readonly ok: true; readonly token: ACPToken }
