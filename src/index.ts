...
function decodeJsonPart<T>(seg: string): T | null {
  try {
 ...
 */
export async function verifyAcpToken(
  jws: string,
  opts: VerifyOptions,
): Promise<VerifyResult> {
  const parts = jws.split(".");
  if (parts.length !== 3) return fail("malformed_jws");
  const [h, p, s] = parts as [string, string, string];

  const header = decodeJsonPart<{ alg?: string; kid?: string; typ?: string }>(h);
  if (!header || !header.alg || !header.kid) return fail("malformed_jws");
  if (!SUPPORTED_ALG.has(header.alg)) return fail("unsupported_alg");
  ...
  const algParams: AlgorithmIdentifier | EcdsaParams =
    header.alg === "EdDSA" ? { name: "Ed25519" } : { name: "ECDSA", hash: "SHA-256" };
  let valid = false;
  try {
  ...
  const body = decodeJsonPart<Record<string, unknown>>(p);
  if (!body || typeof body !== "object") return fail("schema_invalid");

  const token = body as unknown as ACPToken;
  if (
  ...
  if (!opts.issuers.includes(token.issuer)) return fail("issuer_not_allowed");
  if (token.audience !== opts.audience) return fail("audience_mismatch");

  const now = (opts.now ?? (() => Math.floor(Date.now() / 1000)))();
  const skew = opts.clockSkewSec ?? 30;
  if (now + skew < token.nbf) return fail("not_yet_valid");
  if (now - skew > token.exp) return fail("expired");
  ...
  if (opts.maxAgeSec !== undefined) {
    if (typeof token.iat !== "number") return fail("replayed");
    if (now - skew > token.iat + opts.maxAgeSec) return fail("replayed");
  }
